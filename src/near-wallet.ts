...
export class NearWallet {
  ...
  public constructor(
    ...
    public networkName: string = NearNetwork.testnet
  ) {
    switch (networkName) {
      ...
      case NearNetwork.testnet:
        ...
        break;
      case NearNetwork.mainnet:
        this.mintbaseWallet = new MintbaseWallet({apiKey, chain: Chain.near, networkName: Network.mainnet, contractName: ''});
        break;
      case NearNetwork.testnet:
        this.mintbaseWallet = new MintbaseWallet({apiKey, chain: Chain.near, networkName: Network.testnet, contractName: ''});
        break;
      default:
  ...
  public async mintbaseLogin(): Promise<void> {
    if(!this.mintbaseWallet || !this.mintbaseWallet.api ) throw CannotConnectError.becauseMintbaseNotConnected();

    try {
      await this.mintbaseWallet.connect();
    } catch (error) {
